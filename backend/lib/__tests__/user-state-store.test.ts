/**
 * Unit tests for UserStateStore (user state persistence).
 * Tests get, set, getOrCreate, has, delete.
 */
...
function makeMinimalUser(id: string, checking = 1000): UserProfile {
  return {
    id,
    name: 'Test',
    monthlyIncome: 5000,
...
// Clean up test keys so they don't leak into other tests
[TEST_PREFIX + 'u1', TEST_PREFIX + 'create_me', TEST_PREFIX + 'a', TEST_PREFIX + 'b'].forEach((id) =>
  userStateStore.delete(id)
);
