/**
 * Ingest FinTextQA dataset into vector database (financial reasoning and explanations).
 * If data/fintextqa.jsonl exists it is used; otherwise a small built-in sample is embedded.
 */
...
/** Fallback sample Q&A when no JSONL file is present */
...
async function loadFinTextQAFromFile(dataDir: string): Promise<Array<{ question: string; context: string; answer: string; category?: string; difficulty?: string }>> {
  const filePath = path.join(dataDir, 'fintextqa.jsonl');
  try {
    const content = await fs.readFile(filePath, 'utf-8');
    const lines = content.trim().split('\n').filter(Boolean);
    return lines.map(line => JSON.parse(line) as { question: string; context: string; answer: string; category?: string; difficulty?: string });
  } catch {
    return [];
  }
}

async function processFinTextQA(): Promise<Document[]> {
...
export async function ingestFinTextQA(): Promise<void> {
  try {
