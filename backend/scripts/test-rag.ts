 ...
 * 
 * Tests the vector store, user history indexing, knowledge base, and retrieval functionality.
 */
...
async function testRAG() {
  ...
  try {
    ...
    const kbResults = await knowledgeBase.search(
      'emergency fund liquidity best practices',
      3
    );
    ...
    // Additional test: Investment-related query
    console.log('[Bonus] Testing investment-related query...');
    console.log('Query: "time horizon risk tolerance asset allocation"\n');
    
    const investResults = await knowledgeBase.search(
      'time horizon risk tolerance asset allocation',
      3
    );
