...
console.log('-'.repeat(70));
{
    ...
    try {
        ...
        if (recommendations.length > 0) {
            assert(recommendations.some(r => r.action.type === 'save'), 'Generates generic save recommendation');
        }
        else {
...
}
{
    ...
    try {
        ...
        assert(largeRecs.length === 0, 'No large recommendations when surplus is negative', `Found ${largeRecs.length} large recommendations`);
    }
    catch (e) {
...
}
{
    ...
    assert(goalSpecificRecs.length === 0, 'No goal-specific recs when all goals done', `Found ${goalSpecificRecs.length} goal-specific recs`);
}
{
...
}
{
    ...
    assert(unrealistic.length === 0, 'No unrealistic recommendations for huge goal', `Found recommendation for $${unrealistic[0]?.action.amount}`);
}
{
    ...
    assert(!recommendations.some(r => r.action.type === 'invest'), 'Does not recommend investing over emergency fund');
}
{
...
}
{
    ...
    try {
        ...
        assert(recommendations.length === 0 || recommendations.every(r => r.action.amount <= 0), 'Does not recommend actions with no income');
    }
    catch (e) {
...
}
{
    ...
    assert(investRecs.length > 0 || recommendations.length > 0, 'Handles long-term only goals');
}
{
...
}
{
    ...
    if (summary[0]) {
        assert(summary[0].status === 'completed', '100% progress = completed status', `Got: ${summary[0].status}`);
...
}
{
    ...
    if (summary[0]) {
        ...
        assert(summary[0].remainingAmount === 0, 'Remaining amount is 0', `Got: ${summary[0].remainingAmount}`);
    }
    else {
        assert(false, 'Summary should have at least one entry');
    }
}
{
    ...
    try {
        ...
        assert(isFinite(progress) || status === 'completed', 'Handles $0 target gracefully', `Progress: ${progress}`);
    }
    catch (e) {
...
}
{
    ...
    if (summary[0]) {
        assert(summary[0].monthsRemaining > 500, 'Correctly calculates distant months', `Got: ${summary[0].monthsRemaining} months`);
        assert(summary[0].monthlyNeeded > 0, 'Monthly needed is positive', `Got: ${summary[0].monthlyNeeded}`);
    }
    else {
        assert(false, 'Summary should have at least one entry');
    }
}
{
    ...
    if (summary[0]) {
        assert(summary[0].monthsRemaining <= 1, 'Tomorrow = 0 or 1 months', `Got: ${summary[0].monthsRemaining}`);
        assert(summary[0].status === 'at_risk' || summary[0].status === 'behind', 'Tomorrow deadline with gap is at_risk or behind', `Got: ${summary[0].status}`);
    }
    else {
        assert(false, 'Summary should have at least one entry');
    }
}
{
    ...
    if (summary[0] && summary[1]) {
        assert(summary[0].deadline.getTime() === summary[1].deadline.getTime(), 'Same deadlines preserved');
    }
}
{
...
//# sourceMappingURL=recommendation-edge-cases.test.js.map