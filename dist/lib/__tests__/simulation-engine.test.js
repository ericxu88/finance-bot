import { calculateFutureValue, calculateGoalImpact, simulate_save, simulate_invest, compare_options, calculateBudgetStatus, DEFAULT_ANNUAL_RETURN, } from '../simulation-engine.js';
import { sampleUser } from '../sample-data.js';
import { getInvestmentBalance } from '../../types/financial.js';
function assertApproximately(actual, expected, tolerance = 1) {
...
assert(saveResult.scenarioIfDo.accountsAfter.checking === 2500, 'Checking should decrease from $3,000 to $2,500');
assert(saveResult.scenarioIfDo.accountsAfter.savings === 8500, 'Savings should increase from $8,000 to $8,500');
assert(saveResult.scenarioIfDo.goalImpacts.length > 0, 'Should have goal impacts');
const emergencyGoalImpact = saveResult.scenarioIfDo.goalImpacts[0];
assert(emergencyGoalImpact?.goalId === 'goal_emergency', 'Goal impact should be for emergency fund');
assert(emergencyGoalImpact !== undefined && emergencyGoalImpact.progressChangePct > 0, 'Progress should increase');
assert(saveResult.validationResult.passed === true, 'Validation should pass (no guardrail violations)');
...
console.log('='.repeat(60));
const investResult = simulate_invest(sampleUser, 500, 'taxable', 'goal_house', 5);
...
assert(investResult.scenarioIfDo.accountsAfter.checking === 2500, 'Checking should decrease by $500');
assert(getInvestmentBalance(investResult.scenarioIfDo.accountsAfter.investments.taxable) === 5500, 'Taxable investment should increase from $5,000 to $5,500');
...
const houseGoalImpact = investResult.scenarioIfDo.goalImpacts.find(gi => gi.goalId === 'goal_house');
assert(houseGoalImpact !== undefined, 'Should have house goal impact');
assert(houseGoalImpact.progressChangePct > 0, 'House goal progress should increase');
assert(investResult.scenarioIfDo.liquidityImpact.includes('liquidity') ||
    investResult.scenarioIfDo.liquidityImpact.includes('Checking'), 'Should mention liquidity impact');
console.log('✅ Checking: $3,000 → $2,500');
...
assert(violationResult.validationResult.passed === false, 'Validation should fail due to guardrail violation');
assert(violationResult.validationResult.constraintViolations.length > 0, 'Should have constraint violations');
const violation = violationResult.validationResult.constraintViolations[0];
assert(violation !== undefined && violation.includes('1,000') || violation.includes('1000'), 'Violation should mention minimum checking balance of $1,000');
...
assert(comparisonResults.length === 3, 'Should return 3 simulation results');
assert(comparisonResults[0]?.action.type === 'save', 'First result should be save action');
assert(comparisonResults[1]?.action.type === 'invest', 'Second result should be invest action');
assert(comparisonResults[2]?.action.type === 'spend', 'Third result should be spend action');
console.log('\n📊 Comparison Results:');
console.log('-'.repeat(60));
