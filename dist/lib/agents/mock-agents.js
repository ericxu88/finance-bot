export function generateMockBudgetingAnalysis(context) {
    ...
    return {
        ...
        budgeting_metrics: {
            ...
            over_budget_categories: user.spendingCategories
                ...
                .map(c => c.name),
            budget_utilization_pct: user.spendingCategories.reduce((sum, c) => sum + (c.currentSpent / c.monthlyBudget) * 100, 0) / user.spendingCategories.length
        }
    };
}
export function generateMockInvestmentAnalysis(context) {
    const { user, action, simulationResult } = context;
    const goalImpact = simulationResult.scenarioIfDo.goalImpacts[0];
    const futureValue = goalImpact?.futureValue || action.amount * 1.4;
    let recommendation;
    let confidence = 0.8;
    ...
    return {
        ...
        investment_metrics: action.type === 'invest' ? {
            ...
            risk_assessment: (user.preferences.riskTolerance === 'conservative' ? 'low' : 'medium'),
            goal_alignment_score: goalImpact ? 0.85 : 0.6,
            diversification_impact: 'Increases exposure to growth assets'
        } : {}
    };
}
export function generateMockGuardrailAnalysis(context) {
    ...
    for (const guardrail of user.preferences.guardrails) {
        if (guardrail.type === 'min_balance' && guardrail.accountId && guardrail.threshold) {
            const accountBalance = accountsAfter[guardrail.accountId];
...
function generateUserFriendlySummary(context, overallRecommendation, budgetingAnalysis, investmentAnalysis, guardrailAnalysis) {
...
export function generateMockValidationAnalysis(context, budgetingAnalysis, investmentAnalysis, guardrailAnalysis) {
    ...
    return {
        ...
        decision_tree: {
            if_proceed: `If you proceed: ${context.simulationResult.scenarioIfDo.goalImpacts.map((g) => `${g.goalName} progress increases by ${g.progressChangePct.toFixed(1)}%`).join(', ')}. Checking balance becomes $${context.simulationResult.scenarioIfDo.accountsAfter.checking.toFixed(0)}.`,
