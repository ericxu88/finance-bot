...
export class FinancialKnowledgeBase {
    static COLLECTION_NAME = 'financial_knowledge';
    async initialize() {
    ...
    async search(query, k = 3) {
        const docs = await vectorStore.search(FinancialKnowledgeBase.COLLECTION_NAME, query, k);
        return docs.map(doc => ({
            principle: doc.pageContent,
            source: doc.metadata.source || 'Internal knowledge base',
