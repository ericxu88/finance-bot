...
function buildContext(action) {
    const simulationResult = action.type === 'save'
        ? simulate_save(sampleUser, action.amount, action.goalId)
        : action.type === 'invest'
            ? simulate_invest(sampleUser, action.amount, action.targetAccountId ?? 'taxable', action.goalId, 5)
            : (() => {
...
function formatOutput(result) {
    ...
    if (result.budgetingAnalysis.reasoning)
        lines.push('    Reasoning: ' + result.budgetingAnalysis.reasoning.slice(0, 200) + (result.budgetingAnalysis.reasoning.length > 200 ? '...' : ''));
    ...
    if (result.investmentAnalysis.reasoning)
        lines.push('    Reasoning: ' + result.investmentAnalysis.reasoning.slice(0, 200) + (result.investmentAnalysis.reasoning.length > 200 ? '...' : ''));
    lines.push('  Guardrail Agent: ' + (result.guardrailAnalysis.can_proceed ? 'PASS' : 'BLOCKED') + ' (violated: ' + result.guardrailAnalysis.violated + ')');
    const decision = result.finalDecision ?? (result.shouldProceed ? 'proceed' : 'do_not_proceed');
    const consensus = result.consensusLevel ?? (result.validationAnalysis?.agent_consensus?.consensus_level ?? '');
...
async function runOne(queryLabel, action, useRealOrchestrator) {
    console.log('\n' + '═'.repeat(80));
...
//# sourceMappingURL=demo-e2e-workflow.js.map