...
function userToJson(u) {
    return {
        ...
        goals: u.goals.map((g) => ({ ...g, deadline: g.deadline.toISOString() })),
    };
}
...
async function run() {
    ...
    const updated = r.body.updatedUser;
    ...
    assert(r.body.id === 'new_user_never_seen', 'new user should be seeded with id');
    ...
    assert(r.body.error?.includes('Nothing to undo'), 'error message');
    ...
    assert(typeof r.body.frozen === 'boolean', 'GET /freeze returns frozen boolean');
    r = await request('POST', '/freeze', { frozen: true });
    assert(r.status === 200, 'POST /freeze should 200');
    assert(r.body.frozen === true, 'POST /freeze sets frozen');
    r = await request('GET', '/freeze');
    assert(r.body.frozen === true, 'GET /freeze after POST returns true');
...
//# sourceMappingURL=execute-api.test.js.map