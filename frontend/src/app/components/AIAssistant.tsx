...
interface ActionButton {
  label: string;
  page: 'spending' | 'investment' | 'goals';
  icon?: string;
}
...
interface AIAssistantProps {
  isOpen: boolean;
  onToggle: () => void;
  onNavigate?: (page: 'spending' | 'investment' | 'goals') => void;
  onActionNavigate?: (action: string, description: string, amount: number, target?: string) => void;
}

export function AIAssistant({ isOpen, onToggle, onNavigate, onActionNavigate }: AIAssistantProps) {
  ...
  const generateResponse = async (userMessage: string): Promise<Message> => {
    ...
    if (intent.showProgress) {
      return {
        ...
        showProgress: true,
        actionButtons: [
          { label: 'View Progress', page: 'goals', icon: 'chart' },
        ],
        timestamp: new Date(),
      };
    }
...
You're currently spending ${((totalExpenses / state.monthlyIncome) * 100).toFixed(1)}% of your income, which leaves you with ${((available / state.monthlyIncome) * 100).toFixed(1)}% for your financial goals.`,
        showBudget: true,
        actionButtons: [
          { label: 'View Budget', page: 'spending', icon: 'dollar-sign' },
        ],
        timestamp: new Date(),
      };
    }
...
What would you like to explore?`,
  ...
  const handleSend = async () => {
    ...
    // Try backend API first, fall back to local processing
    try {
      ...
      try {
        ...
        console.error('Error generating response:', localError);
        setMessages((prev) => [
          ...prev,
          {
            id: Date.now().toString(),
                    ...
                    {message.options && message.options.length > 0 && (
                      <div className="mt-3 space-y-2 border-t border-gray-200 pt-3">
                        ...
                        {message.options.map((option, idx) => (
                          ...
                          >
                            <div className="text-[11px] p-2 bg-gray-50 rounded border border-gray-200">
                              <div className="font-medium text-[#005EB8] mb-1">{option.action}</div>
                              <div className="text-gray-600 text-[10px] space-y-0.5 mb-2">
                              ...
                              {/* Action button for this option */}
                              <button
                                onClick={() => {
                                  if (onActionNavigate) {
                                    // Extract target from option description or action
                                    const targetMatch = option.action.match(/to\s+(\w+\s*\w*)/i);
                                    const target = targetMatch ? targetMatch[1].toLowerCase().replace(/\s+/g, '') : undefined;
                                    
                                    // Extract amount from action
                                    const amountMatch = option.action.match(/\$?(\d+(?:,\d{3})*(?:\.\d{2})?)/);
                                    const amount = amountMatch ? parseFloat(amountMatch[1].replace(/,/g, '')) : 0;
                                    
                                    onActionNavigate(option.action, option.description, amount, target);
                                    onToggle(); // Close assistant
                                  }
                                }}
                                className="w-full text-[11px] px-3 py-2 bg-[#005EB8] hover:bg-[#004d8c] text-white rounded transition-colors font-semibold flex items-center justify-between"
                              >
                                <span>Take this action</span>
                      ...
                      <div className="mt-3 space-y-2 border-t border-gray-200 pt-3">
                        {message.actionButtons.map((button, idx) => (
                          <button
                            ...
                            onClick={() => {
                              if (onNavigate) {
                                onNavigate(button.page);
                                onToggle(); // Close assistant after navigation
                              }
                            }}
                            className="w-full text-left text-[12px] p-3 bg-[#005EB8] hover:bg-[#004d8c] text-white rounded-lg transition-colors font-semibold flex items-center justify-between"
                          >
              ...
              <div className="flex justify-start">
                <div className="bg-white rounded-lg p-3 shadow-sm">
                  <div className="flex items-center gap-2">
                    ...
                    {statusMessage && (
                      <span className="text-[11px] text-gray-500">{statusMessage}</span>
                    )}
                  </div>
                </div>
              </div>
