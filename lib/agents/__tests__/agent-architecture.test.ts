/**
 * Agent architecture unit tests (no API calls)
 *
 * Verifies:
 * - Mock orchestrator returns valid OrchestrationResult shape
 * - All analysis outputs satisfy Zod schemas
 * - Agent context and result contract are correct
 *
 * Run: npm run build && node dist/lib/agents/__tests__/agent-architecture.test.js
 */
...
const VALID_CONFIDENCE = ['high', 'medium', 'low', 'very_low'] as const;
...
async function main() {
  ...
  assert(VALID_CONFIDENCE.includes(result.overallConfidence as (typeof VALID_CONFIDENCE)[number]), 'Valid overallConfidence');
  assert(typeof result.executionTime === 'number', 'Has executionTime number');
  assert(['blocked', 'do_not_proceed', 'proceed_with_caution', 'proceed'].includes((result as { finalDecision?: string }).finalDecision ?? ''), 'Has finalDecision (policy)');
  assert(['blocked', 'unanimous', 'divided'].includes((result as { consensusLevel?: string }).consensusLevel ?? ''), 'Has consensusLevel (policy)');
  ...
  assert(VALID_RECOMMENDATIONS.includes(b.recommendation as (typeof VALID_RECOMMENDATIONS)[number]), 'Budgeting recommendation enum');
  ...
  assert(VALID_RECOMMENDATIONS.includes(inv.recommendation as (typeof VALID_RECOMMENDATIONS)[number]), 'Investment recommendation enum');
  ...
  assert(
    VALID_OVERALL_RECOMMENDATIONS.includes(v.overall_recommendation as (typeof VALID_OVERALL_RECOMMENDATIONS)[number]),
    'Validation overall_recommendation enum'
  );
  assert(VALID_CONFIDENCE.includes(v.overall_confidence as (typeof VALID_CONFIDENCE)[number]), 'Validation overall_confidence');
  ...
  // --- Consistency ---
  assert(
    result.overallConfidence === v.overall_confidence,
    'Result overallConfidence matches validationAnalysis.overall_confidence'
  );
  assert(
    result.finalRecommendation.startsWith(v.final_summary.trimEnd()),
    'Result finalRecommendation starts with validationAnalysis.final_summary (may append decision line)'
  );
  console.log('  ✅ Result consistency (confidence, final summary)');

  // --- Agents work together: consensus matches upstream analyses ---
  ...
  assert(
    v.agent_consensus.agents_approving + v.agent_consensus.agents_cautioning + v.agent_consensus.agents_opposing === 2,
    'Consensus counts sum to 2 (budgeting + investment)'
  );
  console.log('  ✅ Consensus consistent with budgeting + investment (agents work together)');

  // --- Guardrail violation → shouldProceed blocked ---
  const violationAction = { type: 'save' as const, amount: 2500, goalId: sampleUser.goals[0]?.id ?? 'goal_emergency' };
  ...
  assert(
    (violationResult as { finalDecision?: string }).finalDecision === 'blocked',
    'Policy finalDecision is blocked when guardrail violated'
  );
  console.log('  ✅ Guardrail violation → shouldProceed false, finalDecision blocked (orchestrator contract)');

  // --- Error handling: invalid context throws ---
  let invalidContextThrew = false;
  try {
    await orchestrator.processDecision({} as Parameters<MockAgentOrchestrator['processDecision']>[0]);
  } catch {
