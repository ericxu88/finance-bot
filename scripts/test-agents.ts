/**
 * Comprehensive agent architecture test script
 *
 * Verifies:
 * - Agents work together: all 4 agents (Budgeting, Investment, Guardrail, Validation) run;
 *   validation uses their outputs; guardrail blocks → shouldProceed false; consensus counts match.
 * - Agents provide a response: finalRecommendation present and non-empty; structured outputs
 *   pass Zod schemas; overallConfidence and shouldProceed are valid.
 *
 * Steps:
 * 0. Architecture check (offline) – modules and exports
 * 1. API key check (OpenAI or Gemini)
 * 2. LLM API connectivity (optional skip)
 * 3. Mock orchestrator (offline) – no API, validates pipeline shape
 * 4. Live orchestrator – real 4-agent run, validates result
 * 5. Result contract – strict validation of live result
 * 6. Context sensitivity (optional) – RUN_CONTEXT_SENSITIVITY=1 to enable
 *
...
async function checkArchitecture(): Promise<boolean> {
  ...
  try {
    ...
    const orchestratorMod = (await import(orchestratorUrl)) as { LangChainAgentOrchestrator: new () => { processDecision: (ctx: unknown) => Promise<unknown> } };
    const mockMod = (await import(mockUrl)) as { MockAgentOrchestrator: new () => { processDecision: (ctx: unknown) => Promise<unknown> } };
    ...
    if (ok) console.log('  ✅ All agent modules present and exports valid\n');
    return ok;
  } catch (e: unknown) {
...
function getAgentProvider(): 'openai' | 'gemini' {
  ...
  try {
    ...
    const text =
      typeof raw === 'string'
        ? raw
        : Array.isArray(raw)
          ? String((raw as { text?: string }[])?.[0]?.text ?? (raw as unknown[])?.[0] ?? '')
          : String(raw ?? '');
...
async function verifyGeminiConnection(): Promise<boolean> {
...
async function buildContext(): Promise<{ user: unknown; action: { type: 'save'; amount: number; goalId: string }; simulationResult: unknown; historicalMetrics: Record<string, unknown> }> {
  const sampleMod = await import(pathToFileURL(join(DIST, 'lib', 'sample-data.js')).href) as { sampleUser: { goals: { id: string }[] } };
  const simEngineMod = await import(pathToFileURL(join(DIST, 'lib', 'simulation-engine.js')).href) as { simulate_save: (u: unknown, amount: number, goalId?: string) => unknown };
  const user = sampleMod.sampleUser;
  const goalId = user.goals[0]?.id ?? 'goal_emergency';
  const action = { type: 'save' as const, amount: 100, goalId };
...
async function runMockOrchestratorTest(): Promise<boolean> {
  ...
  try {
    ...
    const result = (await orchestrator.processDecision(context)) as {
      budgetingAnalysis?: { recommendation: string; confidence: number };
      investmentAnalysis?: unknown;
      guardrailAnalysis?: { can_proceed: boolean };
      validationAnalysis?: { overall_recommendation: string; overall_confidence: string; final_summary: string };
...
async function runLiveOrchestratorTest(): Promise<unknown | null> {
  ...
  try {
    ...
    const rawResult = await Promise.race([
      ...
      new Promise<never>((_, reject) => setTimeout(() => reject(new Error('Orchestrator timed out after 180s')), 180_000)),
    ]);
    const result = rawResult as {
      budgetingAnalysis?: { recommendation: string; confidence: number; reasoning?: string };
      investmentAnalysis?: { recommendation: string };
      guardrailAnalysis?: { can_proceed: boolean };
      validationAnalysis?: { overall_recommendation: string; overall_confidence: string; final_summary: string };
      finalRecommendation?: string;
...
async function validateLiveResultZodSchemas(result: unknown): Promise<boolean> {
  section('5a. Live result Zod schema validation');
  try {
    const schemasMod = (await import(pathToFileURL(join(AGENTS, 'schemas.js')).href)) as {
      BudgetingAnalysisSchema: { safeParse: (v: unknown) => { success: boolean; error?: unknown } };
      InvestmentAnalysisSchema: { safeParse: (v: unknown) => { success: boolean; error?: unknown } };
      GuardrailAnalysisSchema: { safeParse: (v: unknown) => { success: boolean; error?: unknown } };
      ValidationAnalysisSchema: { safeParse: (v: unknown) => { success: boolean; error?: unknown } };
    };
    const r = result as {
      budgetingAnalysis?: unknown;
      investmentAnalysis?: unknown;
      guardrailAnalysis?: unknown;
      validationAnalysis?: unknown;
    };
    const b = schemasMod.BudgetingAnalysisSchema.safeParse(r.budgetingAnalysis);
    if (!b.success) {
      console.log('  ❌ Live budgetingAnalysis failed Zod parse:', (b as { error: unknown }).error);
      return false;
    }
    const inv = schemasMod.InvestmentAnalysisSchema.safeParse(r.investmentAnalysis);
    if (!inv.success) {
      console.log('  ❌ Live investmentAnalysis failed Zod parse:', (inv as { error: unknown }).error);
      return false;
    }
    const g = schemasMod.GuardrailAnalysisSchema.safeParse(r.guardrailAnalysis);
    if (!g.success) {
      console.log('  ❌ Live guardrailAnalysis failed Zod parse:', (g as { error: unknown }).error);
      return false;
    }
    const v = schemasMod.ValidationAnalysisSchema.safeParse(r.validationAnalysis);
    if (!v.success) {
      console.log('  ❌ Live validationAnalysis failed Zod parse:', (v as { error: unknown }).error);
      return false;
    }
...
function validateResultContract(result: unknown): boolean {
  ...
  const r = result as {
    ...
    investmentAnalysis?: { recommendation: string; confidence: number };
    guardrailAnalysis?: { can_proceed?: boolean; violations: unknown[]; compliance_summary?: string };
    validationAnalysis?: {
      final_summary: string;
      agent_consensus?: { consensus_level: string; agents_approving: number; agents_cautioning: number; agents_opposing: number };
    };
    finalDecision?: string;
    consensusLevel?: string;
    shouldProceed?: boolean;
  ...
  if (ok) console.log('  ✅ All result contract checks passed\n');
  return ok;
}

async function runContextSensitivityTest(): Promise<boolean> {
  ...
  try {
    ...
    const saveResult = (await orchestrator.processDecision({ ...baseContext, action: { type: 'save' as const, amount: 100, goalId: baseContext.action.goalId } })) as { finalRecommendation?: string; budgetingAnalysis?: { recommendation: string } };
    ...
    const goalIdInvest = investContext.user && typeof investContext.user === 'object' && 'goals' in investContext.user
      ? (investContext.user as { goals?: { id: string }[] }).goals?.[1]?.id ?? investContext.action.goalId
      : investContext.action.goalId;
    const simEngineMod = await import(pathToFileURL(join(DIST, 'lib', 'simulation-engine.js')).href) as { simulate_invest: (u: unknown, a: number, acc: string, g: string, y: number) => unknown };
    ...
    const investResult = (await orchestrator.processDecision({
      ...investContext,
      action: { type: 'invest' as const, amount: 100, goalId: goalIdInvest },
      simulationResult: investSim,
    })) as { finalRecommendation?: string; budgetingAnalysis?: { recommendation: string } };
...
async function main() {
  ...
  if (!(await checkArchitecture())) process.exit(1);
  if (!(await checkApiKey())) process.exit(1);
  ...
  if (skipPing) {
    ...
    if (!(await verifyOpenAIConnection())) process.exit(1);
  } else if (!(await verifyGeminiConnection())) process.exit(1);

  if (!(await runMockOrchestratorTest())) process.exit(1);

  const liveResult = await runLiveOrchestratorTest();
  if (liveResult == null) process.exit(1);
  if (!(await validateLiveResultZodSchemas(liveResult))) process.exit(1);
  if (!validateResultContract(liveResult)) process.exit(1);
  ...
  if (runContextSensitivity) {
    if (!(await runContextSensitivityTest())) process.exit(1);
  } else {
  ...
  if (runContextSensitivity) console.log('  (6) Context sensitivity.');
  console.log('');
  process.exit(0);
}
